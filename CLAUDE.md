...
- **Do not auto-subscribe to PR activity.** Still open a (draft) PR after every push, as usual — just don't call `subscribe_pr_activity` / auto-watch it afterward, and don't treat "create a PR" as an implicit request to monitor it. Subscribing relays *every* GitHub event on that PR into the session as a wake-up, including third-party bot comments (Vercel deploy-status pings, etc.), which is noisy and mostly irrelevant. Only subscribe when a human explicitly asks to have a specific PR watched/monitored/babysat — and unsubscribe once it merges or closes, same as normal.
