...
const ROW_KEYS = ['A', 'B', 'C', 'D', 'E', 'F', 'G'] as const;
...
const COLUMNS = ['central_tax', 'state_tax', 'integrated_tax', 'cess', 'interest', 'penalty'] as const;
type Column = typeof COLUMNS[number];
const COLUMN_LABEL: Record<Column, string> = {
  central_tax: 'Central Tax', state_tax: 'State/UT Tax', integrated_tax: 'Integrated Tax',
  cess: 'Cess', interest: 'Interest', penalty: 'Penalty',
};

type Row = Record<Column, string>;
const emptyRow = (): Row => ({ central_tax: '0', state_tax: '0', integrated_tax: '0', cess: '0', interest: '0', penalty: '0' });
 ...
 * 27 Aug 2026). Fixed 7-row grid, no dependency on anything computed
 * elsewhere — standalone manual entry.
 */
export const Table15DemandsRefundsCard: React.FC<Props> = ({ clientId, financialYear }) => {
  ...
  const load = async () => {
    ...
    const { data, error } = await supabase.from('gstr9_table15_demands_refunds')
      .select('row_key, central_tax, state_tax, integrated_tax, cess, interest, penalty')
      .eq('client_id', clientId).eq('financial_year', financialYear);
    if (error) { toast.error('Could not load Table 15: ' + error.message); setLoading(false); return; }
    const next = {} as Record<RowKey, Row>;
    ROW_KEYS.forEach((k) => { next[k] = emptyRow(); });
    (data || []).forEach((r: { row_key: RowKey } & Record<Column, number>) => {
      next[r.row_key] = { central_tax: String(r.central_tax), state_tax: String(r.state_tax), integrated_tax: String(r.integrated_tax), cess: String(r.cess), interest: String(r.interest), penalty: String(r.penalty) };
    });
    setRows(next);
  ...
  const saveAll = async () => {
    ...
    try {
      ...
      const payload = dirtyKeys.map((k) => ({
        ...
        central_tax: num(rows[k].central_tax), state_tax: num(rows[k].state_tax), integrated_tax: num(rows[k].integrated_tax),
        cess: num(rows[k].cess), interest: num(rows[k].interest), penalty: num(rows[k].penalty), updated_at: now,
      }));
      const { error } = await supabase.from('gstr9_table15_demands_refunds').upsert(payload, { onConflict: 'client_id,financial_year,row_key' });
  ...
  if (loading) return (
    <Card>
      <CardHeader>
        <CardTitle className="text-lg">TABLE 15 — Demands and Refunds</CardTitle>
...
export default Table15DemandsRefundsCard;
