// The 4 "ready now" GSTR-2A/2B family reports on the Reports Hub. All read
// twob_import_docs (this period's imported GSTR-2B — the only data-entry
// surface for 2B classification, per docs/2B_RECONCILIATION_FLOW.md) or
// books_register (the firm's own purchase register), both keyed the same
// way the rest of Import 2B reads them: client_id + period_month (MM/YYYY,
// no short-label conversion needed here, unlike the GSTR-1 family).
//
// Neither table stores an explicit tax rate — the 2B Excel export and the
// books register both carry only taxable value + tax amounts, no rate
// column (confirmed in parseGstr2b.ts). Rate-wise reports below derive the
// rate from tax ÷ taxable value and snap it to the nearest standard GST
// slab, same approach a manual rate-wise working paper would use.
...
export const buildGstr2bRateWiseReport = async (clientId: string, month: string): Promise<ReportTable> => {
  ...
  const docs = data || [];
  if (docs.length === 0) {
    throw new Error(`No GSTR-2B data imported for ${client.name} in ${formatMonthLabel(month)}. Import it first from Import 2B.`);
  }
  ...
  return {
    ...
    subtitle: `Client: ${client.name}   |   GSTIN: ${client.gstin || '—'}   |   Period: ${formatMonthLabel(month)}   |   Rate is derived from tax ÷ taxable value and snapped to the nearest standard GST slab — the imported GSTR-2B doesn't carry an explicit rate column.`,
...
export const buildGstr2bPyInCyReport = async (clientId: string, month: string): Promise<ReportTable> => {
  ...
  if (rows.length === 0) {
    rows.push(['—', 'No previous-year-dated documents found in this period\'s imported GSTR-2B.', '', '', '', '', '']);
  }
